...
const ManualInvoice = ({token}) => {
  ...
  const downloadPdf = async (invoice, autoName) => {
    ...
    const fileName = match ? match[1] : autoName || `Invoice-${invoice.invoiceNumber || invoice._id}.pdf`
  ...
  const generateInvoice = async () => {
    ...
    try {
      ...
      toast.success('Manual invoice generated and saved')
      try {
        await downloadPdf(invoice, `Invoice-${invoice.invoiceNumber}.pdf`)
      } catch (err) {
        console.log(err)
      }
      resetForm()
  ...
  return (
    <div>
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5'>
        <div>
          <h3 className='text-lg font-semibold text-gray-800'>Manual Invoice Generator</h3>
          <p className='text-sm text-gray-500 mt-0.5'>Create a professional invoice manually using the same template as order invoices.</p>
        </div>
      </div>
        ...
        <div className='flex flex-col gap-5 min-w-0'>
          <div className='bg-white rounded-xl border border-slate-200 shadow-sm p-5'>
            <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4'>
              <p className='text-sm font-semibold text-gray-800'>Select Products</p>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-5 items-start mt-5'>
        <div className='bg-white rounded-xl border border-slate-200 shadow-sm p-5 lg:col-span-2 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
          ...
          <div className='flex flex-col gap-2 sm:w-56 shrink-0'>
            ...
            <button onClick={resetForm} disabled={generating} className='w-full py-3 px-4 border border-slate-300 text-gray-700 text-sm font-semibold rounded-lg hover:border-primary hover:text-primary transition-colors disabled:opacity-60'>
              Reset
            </button>
          </div>
        </div>
